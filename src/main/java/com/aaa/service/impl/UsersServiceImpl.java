...
public class UsersServiceImpl implements UsersService {
    ...
    public Map<String, Object> addUser(Users users) {
        ...
        try {
            ...
            if(users.getCreateTime()==null || users.getCreateTime().toString() == ""){
                users.setCreateTime(new Date());
            }
            users.setLoginName(users.getPhone());
            users.setPassword("123456");
            LxlShiroUtil.encryptPassword(users);
            users.setPath(users.getPath());
            dao.addUser(users);
            //获得主键
        ...
        try {
            ...
            for(Users user : resultList){
                ...
                if(stringObjectMap.get("phoneChangeInfo").equals("合格")){
                    ...
                    user.setRoleName(dao.getRoleByRoleName(user.getRoleName()).get(0).get("id").toString());
                    addUser(user);
                }
            }
            ret=1;
